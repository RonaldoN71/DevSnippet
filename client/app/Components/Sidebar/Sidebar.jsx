...
function Sidebar() {
  ...
  const getIconColor = (url) =>{
    if (pathName === "/login") return "#71717a";
    return pathName === url ? "#aaa" : "#71717a";
  };

  return (
    <div className={`fixed z-20 bg-2 h-full border-r-[2px] border-rgba-3 ${isSidebarOpen ? "w-[15rem]" : "w-[5.2rem]"}`}>
    <span
        ...
        <ul>
          {menu.slice(0,-2).map((item)=>{
            return (
              <li
              key = {item.id}
              className={`sidebar-nav-item my-[.3rem] px-8 py-[.6rem] cursor-pointer ${pathName===item.url && "active-nav-item"}`}
                onClick={()=>{
          ...
          {menu.slice(-2).map((item)=>{
            return (
              ...
              >
                ...
                <span className="text-lg" style={{color: getIconColor(item.url)}}>
                  {item.icon}
                </span>
                <span>{item.name}</span>
                </Link>
        ...
        isSidebarOpen && (
          <footer className="mb-[5rem] p-4 border-t-[2px] border-rgba-3 text-gray-300">
            <ul className="flex items-center justify-center gap-4">
              <li>
                <Link href="/terms" className="underline text-sm hover:text-green-400">
                Terms
                </Link>
              </li>
              <li>
                <Link href="/privacy" className="underline text-sm hover:text-green-400">
                  Privacy
                </Link>
              </li>
              <li>
               <Link href="/help" className="underline text-sm hover:text-green-400">
                  Help
                </Link>               
              </li>
            </ul>
